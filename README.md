# DecodeLabs Internship

## Project 1
- Hosted a static portfolio website using Amazon S3.
- Uploaded project source code to GitHub.

## Project 2
- Launched an Amazon EC2 instance.
- Installed and configured Nginx Web Server.
- Successfully accessed the webpage using the EC2 Public IP.

## Project 3
- Created an Amazon RDS MySQL database.
- Engine: MySQL
- Instance Type: db.t3.micro
- Status: Available
- Port: 3306

## Project 4
- Created an AWS Lambda Serverless Function.
- Developed a Python function to calculate the sum of two numbers.
- Tested the function using AWS Lambda Test Events.
- Verified successful execution using Amazon CloudWatch Logs.

## AWS Services Used
- Amazon S3
- Amazon EC2
- Amazon RDS
- AWS Lambda
- Amazon CloudWatch
- Nginx
- GitHub
